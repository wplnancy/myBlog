...
var menuRender = (function () {
    ...
    function completeScroll() {
        exampleIS = new IScroll('.menuNav', {
            ...
            useTransform: false//->开始TRANSFORM滚动,默认就是开启的,写成FALSE是禁止
        });
    }
...
var calendarRender = (function () {
    ...
    $calendarPlan.add(function (today, data, columnId) {
        ...
        matchRender.init(columnId, startTime, endTime, $link);
    ...
    $calendarPlan.add(function (today, data, columnId) {
        $btn.on('click', function () {
            ...
            $wrapper.stop().animate({left: curL}, 300, function () {
                ...
                var startTime = $link.eq(starIndex).attr('data-time'),
                    endTime = $link.eq(endIndex).attr('data-time');
                //->控制MATCH区域的数据
                matchRender.init(columnId, startTime, endTime, $link);
            });
        ...
        //     console.log($(this).attr("date-time"));
        //     console.log(document.getElementById("demoH2" + $(this).attr("date-time")));
        //     matchScroll.scrollToElement(document.getElementById("demoH2" + $(this).attr("date-time")), 500);
        //
        // });
    });
...
var matchRender = (function () {
    ...
    var $matchPlain = $.Callbacks();
    //计算中间盒子的高度
    var $match = $('.con');
    var $winH = document.documentElement.clientHeight || document.body.clientHeight;

    var $calendar = $('.calendar');
    $matchPlain.add(function () {
    ...
    //绑定数据
    $matchPlain.add(function (columnId, startTime, endTime) {
        $.ajax({
                ...
                if (matchData) {
                    ...
                    for (var key in matchData) {
                        ...
                        for (var i = 0; i < curAry.length; i++) {
                            ...
                            str += "<li>";
                            str += "<div class='demo-left'><span class='startTime'>" + curData["startTime"].myFormatTime("{3}:{4}") + "</span><span class='matchTitle'>" + curData["matchDesc"] + "</span></div>";

                            str += "<div class='demo-center'>";
    ...
    return {
        init: function (columnId, startTime, endTime) {
            ...
            $matchPlain.fire(columnId, startTime, endTime)
        }
    }
})();
